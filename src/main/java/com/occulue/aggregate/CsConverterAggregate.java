...
public class CsConverterAggregate {  
    ...
    public void handle(AssignMaxAlphaToCsConverterCommand command) throws Exception {
    	...
    	if (  maxAlpha != null && maxAlpha.getAngleDegreesId() == command.getAssignment().getAngleDegreesId() )
    		throw new ProcessingException( "MaxAlpha already assigned with id " + command.getAssignment().getAngleDegreesId() );  
    		
        apply(new AssignMaxAlphaToCsConverterEvent(command.getCsConverterId(), command.getAssignment()));
    }
    ...
    public void handle(UnAssignMaxAlphaFromCsConverterCommand command) throws Exception {
    	...
    	if (  maxAlpha == null )
    		throw new ProcessingException( "MaxAlpha already has nothing assigned." );  
    ...
    public void handle(AssignMaxGammaToCsConverterCommand command) throws Exception {
    	...
    	if (  maxGamma != null && maxGamma.getAngleDegreesId() == command.getAssignment().getAngleDegreesId() )
    		throw new ProcessingException( "MaxGamma already assigned with id " + command.getAssignment().getAngleDegreesId() );  
    		
        apply(new AssignMaxGammaToCsConverterEvent(command.getCsConverterId(), command.getAssignment()));
    }
    ...
    public void handle(UnAssignMaxGammaFromCsConverterCommand command) throws Exception {
    	...
    	if (  maxGamma == null )
    		throw new ProcessingException( "MaxGamma already has nothing assigned." );  
    ...
    public void handle(AssignMaxIdcToCsConverterCommand command) throws Exception {
    	...
    	if (  maxIdc != null && maxIdc.getCurrentFlowId() == command.getAssignment().getCurrentFlowId() )
    		throw new ProcessingException( "MaxIdc already assigned with id " + command.getAssignment().getCurrentFlowId() );  
    ...
    public void handle(UnAssignMaxIdcFromCsConverterCommand command) throws Exception {
    	...
    	if (  maxIdc == null )
    		throw new ProcessingException( "MaxIdc already has nothing assigned." );  
    ...
    public void handle(AssignMinAlphaToCsConverterCommand command) throws Exception {
    	...
    	if (  minAlpha != null && minAlpha.getAngleDegreesId() == command.getAssignment().getAngleDegreesId() )
    		throw new ProcessingException( "MinAlpha already assigned with id " + command.getAssignment().getAngleDegreesId() );  
    		
        apply(new AssignMinAlphaToCsConverterEvent(command.getCsConverterId(), command.getAssignment()));
    }
    ...
    public void handle(UnAssignMinAlphaFromCsConverterCommand command) throws Exception {
    	...
    	if (  minAlpha == null )
    		throw new ProcessingException( "MinAlpha already has nothing assigned." );  
    ...
    public void handle(AssignMinGammaToCsConverterCommand command) throws Exception {
    	...
    	if (  minGamma != null && minGamma.getAngleDegreesId() == command.getAssignment().getAngleDegreesId() )
    		throw new ProcessingException( "MinGamma already assigned with id " + command.getAssignment().getAngleDegreesId() );  
    		
        apply(new AssignMinGammaToCsConverterEvent(command.getCsConverterId(), command.getAssignment()));
    }
    ...
    public void handle(UnAssignMinGammaFromCsConverterCommand command) throws Exception {
    	...
    	if (  minGamma == null )
    		throw new ProcessingException( "MinGamma already has nothing assigned." );  
    ...
    public void handle(AssignMinIdcToCsConverterCommand command) throws Exception {
    	...
    	if (  minIdc != null && minIdc.getCurrentFlowId() == command.getAssignment().getCurrentFlowId() )
    		throw new ProcessingException( "MinIdc already assigned with id " + command.getAssignment().getCurrentFlowId() );  
    ...
    public void handle(UnAssignMinIdcFromCsConverterCommand command) throws Exception {
    	...
    	if (  minIdc == null )
    		throw new ProcessingException( "MinIdc already has nothing assigned." );  
    ...
    public void handle(AssignRatedIdcToCsConverterCommand command) throws Exception {
    	...
    	if (  ratedIdc != null && ratedIdc.getCurrentFlowId() == command.getAssignment().getCurrentFlowId() )
    		throw new ProcessingException( "RatedIdc already assigned with id " + command.getAssignment().getCurrentFlowId() );  
    		
        apply(new AssignRatedIdcToCsConverterEvent(command.getCsConverterId(), command.getAssignment()));
    }
    ...
    public void handle(UnAssignRatedIdcFromCsConverterCommand command) throws Exception {
    	...
    	if (  ratedIdc == null )
    		throw new ProcessingException( "RatedIdc already has nothing assigned." );  
