...
public class BasicIntervalScheduleAggregate {  
    ...
    public BasicIntervalScheduleAggregate(CreateBasicIntervalScheduleCommand command) throws Exception {
    	...
    	CreateBasicIntervalScheduleEvent event = new CreateBasicIntervalScheduleEvent(command.getBasicIntervalScheduleId(), command.Value1Unit(), command.Value2Unit());
    ...
    public void handle(UpdateBasicIntervalScheduleCommand command) throws Exception {
    	...
    	UpdateBasicIntervalScheduleEvent event = new UpdateBasicIntervalScheduleEvent(command.getBasicIntervalScheduleId(), command.StartTime(), command.Value1Unit(), command.Value2Unit());        
    ...
    public void handle(AssignStartTimeToBasicIntervalScheduleCommand command) throws Exception {
    	...
    	if (  startTime != null && startTime.getDateTimeId() == command.getAssignment().getDateTimeId() )
    		throw new ProcessingException( "StartTime already assigned with id " + command.getAssignment().getDateTimeId() );  
    		
        apply(new AssignStartTimeToBasicIntervalScheduleEvent(command.getBasicIntervalScheduleId(), command.getAssignment()));
    }
    ...
    public void handle(UnAssignStartTimeFromBasicIntervalScheduleCommand command) throws Exception {
    	...
    	if (  startTime == null )
    		throw new ProcessingException( "StartTime already has nothing assigned." );  

    	apply(new UnAssignStartTimeFromBasicIntervalScheduleEvent(command.getBasicIntervalScheduleId()));
    }
