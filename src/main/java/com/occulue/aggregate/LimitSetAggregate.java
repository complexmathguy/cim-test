...
public class LimitSetAggregate {  
    ...
    public void handle(UpdateLimitSetCommand command) throws Exception {
    	...
    	UpdateLimitSetEvent event = new UpdateLimitSetEvent(command.getLimitSetId(), command.PercentageLimitsFlag());        
    ...
    public void handle(AssignPercentageLimitsFlagToLimitSetCommand command) throws Exception {
    	...
    	if (  percentageLimitsFlag != null && percentageLimitsFlag.getBooleanProxyId() == command.getAssignment().getBooleanProxyId() )
    		throw new ProcessingException( "PercentageLimitsFlag already assigned with id " + command.getAssignment().getBooleanProxyId() );  
    		
        apply(new AssignPercentageLimitsFlagToLimitSetEvent(command.getLimitSetId(), command.getAssignment()));
    }
