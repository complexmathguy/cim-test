...
public class BusbarSectionAggregate {  
    ...
    public void handle(UpdateBusbarSectionCommand command) throws Exception {
    	...
    	UpdateBusbarSectionEvent event = new UpdateBusbarSectionEvent(command.getBusbarSectionId(), command.IpMax());        
    ...
    public void handle(AssignIpMaxToBusbarSectionCommand command) throws Exception {
    	...
    	if (  ipMax != null && ipMax.getCurrentFlowId() == command.getAssignment().getCurrentFlowId() )
    		throw new ProcessingException( "IpMax already assigned with id " + command.getAssignment().getCurrentFlowId() );  
    		
        apply(new AssignIpMaxToBusbarSectionEvent(command.getBusbarSectionId(), command.getAssignment()));
    }
    ...
    public void handle(UnAssignIpMaxFromBusbarSectionCommand command) throws Exception {
    	...
    	if (  ipMax == null )
    		throw new ProcessingException( "IpMax already has nothing assigned." );  
