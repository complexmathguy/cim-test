...
public class PhaseTapChangerNonLinearAggregate {  
    ...
    public PhaseTapChangerNonLinearAggregate(CreatePhaseTapChangerNonLinearCommand command) throws Exception {
    	...
    	CreatePhaseTapChangerNonLinearEvent event = new CreatePhaseTapChangerNonLinearEvent(command.getPhaseTapChangerNonLinearId());
    ...
    public void handle(UpdatePhaseTapChangerNonLinearCommand command) throws Exception {
    	...
    	UpdatePhaseTapChangerNonLinearEvent event = new UpdatePhaseTapChangerNonLinearEvent(command.getPhaseTapChangerNonLinearId(), command.VoltageStepIncrement(), command.XMax(), command.XMin());        
    ...
    @CommandHandler
    public void handle(AssignVoltageStepIncrementToPhaseTapChangerNonLinearCommand command) throws Exception {
    	LOGGER.info( "Handling command AssignVoltageStepIncrementToPhaseTapChangerNonLinearCommand" );
    	
    	if (  voltageStepIncrement != null && voltageStepIncrement.getPerCentId() == command.getAssignment().getPerCentId() )
    		throw new ProcessingException( "VoltageStepIncrement already assigned with id " + command.getAssignment().getPerCentId() );  
    		
        apply(new AssignVoltageStepIncrementToPhaseTapChangerNonLinearEvent(command.getPhaseTapChangerNonLinearId(), command.getAssignment()));
    }

    @CommandHandler
    public void handle(UnAssignVoltageStepIncrementFromPhaseTapChangerNonLinearCommand command) throws Exception {
    	...
    	apply(new UnAssignVoltageStepIncrementFromPhaseTapChangerNonLinearEvent(command.getPhaseTapChangerNonLinearId()));
    }
    @CommandHandler
    public void handle(AssignXMaxToPhaseTapChangerNonLinearCommand command) throws Exception {
    	...
    	if (  xMax != null && xMax.getReactanceId() == command.getAssignment().getReactanceId() )
    		throw new ProcessingException( "XMax already assigned with id " + command.getAssignment().getReactanceId() );  
    		
        apply(new AssignXMaxToPhaseTapChangerNonLinearEvent(command.getPhaseTapChangerNonLinearId(), command.getAssignment()));
    }
    ...
    public void handle(UnAssignXMaxFromPhaseTapChangerNonLinearCommand command) throws Exception {
    	...
    	if (  xMax == null )
    		throw new ProcessingException( "XMax already has nothing assigned." );  

    	apply(new UnAssignXMaxFromPhaseTapChangerNonLinearEvent(command.getPhaseTapChangerNonLinearId()));
    }
    @CommandHandler
    public void handle(AssignXMinToPhaseTapChangerNonLinearCommand command) throws Exception {
    	...
    	if (  xMin != null && xMin.getReactanceId() == command.getAssignment().getReactanceId() )
    		throw new ProcessingException( "XMin already assigned with id " + command.getAssignment().getReactanceId() );  
    		
        apply(new AssignXMinToPhaseTapChangerNonLinearEvent(command.getPhaseTapChangerNonLinearId(), command.getAssignment()));
    }
    ...
    public void handle(UnAssignXMinFromPhaseTapChangerNonLinearCommand command) throws Exception {
    	...
    	if (  xMin == null )
    		throw new ProcessingException( "XMin already has nothing assigned." );  

    	apply(new UnAssignXMinFromPhaseTapChangerNonLinearEvent(command.getPhaseTapChangerNonLinearId()));
    }
