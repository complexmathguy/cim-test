...
public class BaseVoltageAggregate {  
    ...
    public void handle(UpdateBaseVoltageCommand command) throws Exception {
    	...
    	UpdateBaseVoltageEvent event = new UpdateBaseVoltageEvent(command.getBaseVoltageId(), command.NominalVoltage());        
    ...
    public void handle(AssignNominalVoltageToBaseVoltageCommand command) throws Exception {
    	...
    	if (  nominalVoltage != null && nominalVoltage.getVoltageId() == command.getAssignment().getVoltageId() )
    		throw new ProcessingException( "NominalVoltage already assigned with id " + command.getAssignment().getVoltageId() );  
    		
        apply(new AssignNominalVoltageToBaseVoltageEvent(command.getBaseVoltageId(), command.getAssignment()));
    }
