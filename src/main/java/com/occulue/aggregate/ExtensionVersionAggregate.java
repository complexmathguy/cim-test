...
public class ExtensionVersionAggregate {  
    ...
    public ExtensionVersionAggregate(CreateExtensionVersionCommand command) throws Exception {
    	...
    	CreateExtensionVersionEvent event = new CreateExtensionVersionEvent(command.getExtensionVersionId());
    ...
    public void handle(UpdateExtensionVersionCommand command) throws Exception {
    	...
    	UpdateExtensionVersionEvent event = new UpdateExtensionVersionEvent(command.getExtensionVersionId(), command.Date(), command.NamespaceURI());        
    ...
    public void handle(AssignDateToExtensionVersionCommand command) throws Exception {
        ...
        apply(new AssignDateToExtensionVersionEvent(command.getExtensionVersionId(), command.getAssignment()));
    }
    ...
    public void handle(AssignNamespaceURIToExtensionVersionCommand command) throws Exception {
    	...
    	if (  namespaceURI != null && namespaceURI.getStringProxyId() == command.getAssignment().getStringProxyId() )
    		throw new ProcessingException( "NamespaceURI already assigned with id " + command.getAssignment().getStringProxyId() );  
    		
        apply(new AssignNamespaceURIToExtensionVersionEvent(command.getExtensionVersionId(), command.getAssignment()));
    }
